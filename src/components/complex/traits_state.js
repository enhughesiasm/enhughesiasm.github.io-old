...
const traitsState = {
	...
	init(savedValues){
		...
		traitsState.getEmployeeByType(employeeTypes.PRODUCTION).allocation.manual = traitsState.employees[employeeTypes.PRODUCTION].hiredAmount;
		...
		traitsState.machines.all = traitsState.machines.all.sort(orderByMultipleProperties('role', 'baseCost'));
		...
		for(let builtMachineType in traitsState.machinesBuilt){
			...
			if(amount > 0){
				...
				traitsState.getMachineByType(builtMachineType).builtAmount += amount;
		...
		if(values.upgradesPurchased && values.upgradesPurchased.length > 0){
			for(let u in values.upgradesPurchased){
				...
				const upgrade = traitsState.getUpgradeByKey(values.upgradesPurchased[u]);
				if(upgrade){
		...
		let p = poissonProcess.create(traitsState.massDelivery.averageWaitInMilliseconds, traitsState.massDelivery.trigger);
		p.start();

		traitsState.stored = Math.min(traitsState.stored, traitsState.currentStorageSize());

		// reallocate workers for maximum efficiency
		let prodWorkers = traitsState.getEmployeeByType(employeeTypes.PRODUCTION);

		while (prodWorkers.canReallocate(employeeAllocations.MACHINE, traitsState)){
	...
	massDelivery: {
		...
		perform(){
			...
			if(s < 100){
				...
				numberToDeliver = Math.floor(s * .5);
			}else if (s < 1000000){
				numberToDeliver = Math.floor(s * .4);
			}else {
				numberToDeliver = Math.floor(s * .2);
			}
	...
	// per second
	getTimeToMake(){
		let secondsRequired = (traitsState.workRequiredToMakeTrait * 1.0) / (traitsState.makeWorkPerSecond() * traitsState.overallWorkMultiplier);
	...
	getOutcomePerSecond(type){
		...
		if(type == employeeTypes.PRODUCTION){
			...
			traitsState.machines.getBuiltByRole(machineRoles.PRODUCTION).forEach(m => {
	...
	// storage methods
	currentStorageSize(){
		return Math.min(traitsState.getNaturalStorageSize() + traitsState.machines.getMachineStorageSize(), traitsState.storageMax);
	},
	getNaturalStorageSize(){
		return Math.min(traitsState.storageBase * traitsState.storageMultiplier, traitsState.storageMax);
	},
	...
	getUnlockedUpgrades(){
		...
		return traitsState.allUpgrades.filter(u => u.unlocked
	...
	canPurchaseUpgrade(id){
		...
		if(!upgrade){ console.error('missing upgrade in canPurchaseUpgrade, id: ' + id); return false; }
	...
	purchaseUpgrade(id, activatePyro){
		...
		const upgrade = traitsState.allUpgrades.filter(u => u.id == id)[0];
		if(!upgrade){ console.error('missing upgrade in purchaseUpgrade, id: ' + id); return; }

		if(!traitsState.canPurchaseUpgrade(id)){
			console.error('attempting to purchase unpurchasable upgrade: ' + id);
			return;
		}
	...
	applyUpgrade(upgrade, activatePyro){
		...
		if(traitsState.upgradesPurchased.filter(a => a == upgrade.key).length == 0){
	...
	canHireEmployee(id){
		...
		if(!emp){ console.error('missing employee in canHireEmployee, id: ' + id); return false; }
	...
	getEmployeeCost(id){
		const emp = traitsState.allEmployees.filter(u => u.id == id)[0];
		if(!emp){ console.error('missing employee in getEmployeeCost, id: ' + id); return 9999999999990; }

		let status = traitsState.employees[emp.type];
		if (status.hiredAmount == 0 ) return emp.baseCost;

		let cost = (emp.baseCost * status.costMultiplier) * (Math.pow(traitsState.costScalingBaseMultiplier, status.hiredAmount));
		//		let cost = (emp.baseCost * status.costMultiplier) * (Math.pow(status.hiredAmount, traitsState.costScalingExponent));

		return status.hiredAmount < 15 ? Math.ceil(cost/2*2) : Math.ceil(cost/5)*5; // round to nearest 5, for fun
	},
	hireEmployee(id){

		const emp = traitsState.allEmployees.filter(u => u.id == id)[0];
		if(!emp){ console.error('missing employee in hireEmployee, id: ' + id); return false; }
	...
	getFlexibleMultiplier(employeeType){
		if(!traitsState.flexibleMachineryUnlocked || traitsState.flexibleMachinerySetting == 0) return 1;

		switch(employeeType){
		case employeeTypes.PRODUCTION:
			return 1 - (traitsState.flexibleMachinerySetting * traitsState.flexibleMachineFactor);
		case employeeTypes.DELIVERY:
			return 1 + (traitsState.flexibleMachinerySetting * traitsState.flexibleMachineFactor);
		default:
			console.error('called getFlexibleMultiplier with wrong type: ' + employeeType);
		}
	...
	machines: {
		...
		getBuiltByRole(role){
			return traitsState.machines.all.filter(a => a.role == role && a.builtAmount > 0);
		},
		getCountByRole(role){
			let all = traitsState.machines.getBuiltByRole(role);
			if( all.length > 0) {return all.map(a => a.builtAmount || 0).reduce((a,b) => a + b);}
			else{
				return 0;
			}
		},
		totalPower(includeBuildQueue){
			let power = 0;
			traitsState.machines.all.filter(a => a.unlocked && a.builtAmount > 0).forEach(m => {
				power += (m.power * m.builtAmount);
			});

			if(includeBuildQueue){
				traitsState.machines.buildQueue.forEach(buildItem => {
					power += traitsState.machines.all.filter(m =>  m.type==buildItem.type)[0].power;
				});
			}
		...
		spreadAllocation(amount){
			...
			let machines = traitsState.machines.getBuiltByRole(machineRoles.PRODUCTION).sort(a => a.cost).reverse();
		...
		getMachineStorageSize(){
			let size = 0;
			traitsState.machines.all.filter(a => a.builtAmount > 0 && a.role == machineRoles.STORAGE).forEach(m => {
				size += (m.storage() * traitsState.machineStorageMultiplier * m.builtAmount);
			});
			return size;
		},
		sell(machine){
		...
		tick(delta){
			...
			let machines = traitsState.machines.getBuiltByRole(machineRoles.PRODUCTION);
			let workers = traitsState.getEmployeeByType(employeeTypes.PRODUCTION);
			...
			machines.forEach(m => {
				if(m.workers > 0){
					...
					if(workers.work.paused){
						...
						let workPerSecond = 1.0 * traitsState.overallWorkMultiplier * m.baseWorkPerSecond() * m.workers * workers.baseWorkPerSecond * traitsState.employees[employeeTypes.PRODUCTION].workPerSecondMultiplier;

						m.work.progress += (workPerSecond * delta / 1000.0);

						let everFailed = false;
						let outputAmount = m.traitsPerOutput * traitsState.machineOutputMultiplier *traitsState.getFlexibleMultiplier(employeeTypes.PRODUCTION);
						...
						{	m.work.justFailed = true;
							setTimeout(() => m.work.justFailed = false, traitsState.justFailedMs);
						}

						// set the current rate on the platonic employee object
						let timeRequired = (m.work.required * 1.0) / workPerSecond;
	...
	getUnlockedMachines(){
		let totalBuilt = traitsState.machines.all.map(a => a.builtAmount).reduce((a,b) => (a||0)+(b||0), 0);

		if(totalBuilt == 0){
			return traitsState.machines.all.filter(a => a.unlocked && a.type == machineTypes.LEMON_POWER);
		}else{
	...
	canBuildMachine(machine){
		...
		let cost = traitsState.getMachineCost(machine.type);
		let availablePower = Math.min(traitsState.machines.totalPower(false), traitsState.machines.totalPower(true));
	...
	getMachineCost(type){
		...
		if(!machine) console.error('missing machine in getMachineCost: (' + type + ' )');

		let amountInBuildQueue = traitsState.machines.buildQueue.filter(a => a.type == type).length;

		let totalAmount = machine.builtAmount + amountInBuildQueue;

		if (totalAmount == 0 ) return (machine.baseCost * traitsState.machineCostMultiplier);

		let cost = machine.baseCost * traitsState.machineCostMultiplier * (Math.pow(traitsState.costScalingBaseMultiplier, totalAmount));
	...
	addMachineToQueue(machine, cost){
		...
		traitsState.machines.buildQueue.push( { type: machine.type, cost: cost, name: machine.name  });
	...
	cancelMachineFromQueue(entry, index){
		...
		if(index == 0){
			...
			traitsState.getEmployeeByType(employeeTypes.BUILDER).work.building = machineTypes.NONE;
			traitsState.getEmployeeByType(employeeTypes.BUILDER).work.progress = 0;
		}
	},
	...
	getCurrentlyBuildingMachine(){
		...
		if(builder && builder.work){
			return traitsState.getMachineByType(builder.work.building) ||  traitsState.getMachineByType(machineTypes.NONE);
		}
	...
	buildMachine(machine, amount){
		...
		if(!bqItem){
			console.error('missing buildQueueItem in buildMachine: (' + machine.type + ' )');
			return;
		}

		if(bqItem.type != machine.type){
			console.error('incorrect type for buildQueueItem in buildMachine: (' + machine.type + ' )');
			return;
		}
	...
	// making
	onMakeTraitsClicked(){
		if(traitsState.isMakingTraits){ return;}
	...
	progressMakingTraits(delta){
		...
		if(traitsState.makeProgressPercent >= 100){
			...
			if(!success && !traitsState.justFailedToMake){
				...
				setTimeout(() => traitsState.justFailedToMake = false, traitsState.justFailedMs);
			}
			return;
		}
		...
		traitsState.makeProgressPercent += ((amountToProgress * 1.0 / (traitsState.workRequiredToMakeTrait * 1.0 / (traitsState.makeWorkPerSecond() * traitsState.overallWorkMultiplier))) * 100);
	},
	canMakeTraits(){
	...
	makeTraits(amount){
		...
		if(!traitsState.isStorageFull()){
			...
			traitsState.totalWastedProduction += Math.max(amount - amountMade, 0);
			return amountMade;
		}else{
	...
	updateVisualStorage(){
		...
		if(traitsState.storedTraits.filter(a => !a.delivered).length == traitsState.stored) return;

		let desiredAmount = Math.min(traitsState.visualStorageMax, traitsState.stored);

		let currentAmount = (traitsState.stored <= traitsState.visualStorageMax &&
			...
			for(let i = 0; i < (desiredAmount - currentAmount); i++){
				...
				const newTrait = {
					...
					'deliver': function(){
						...
						setTimeout(() => traitsState.removeFromVisualStorage(this.id), traitsState.deliveryFadeOutTimeMs);
					}
				};
	...
	addToStorage(amount){
		...
		traitsState.stored = Math.min(traitsState.stored + amount, traitsState.currentStorageSize());
	...
	// delivery
	getDeliverySpeed(){
		return traitsState.deliveryRateBase * traitsState.deliveryRateMultiplier;
	},
	autoDeliverTraits(amount){
		...
		if( amountRemaining >= traitsState.visualStorageMax){
			...
			let undeliveredVisuals = traitsState.storedTraits.filter(a => !a.delivered);
			...
			let visualsToAffect = traitsState.storedTraits.filter(a => !a.delivered).length - amountRemaining;

			let affectedVisuals = traitsState.storedTraits.filter(a => !a.delivered).slice(0, visualsToAffect); // gets all elements from amount remaining to end

			if(visualsToAffect != affectedVisuals.length || visualsToAffect == 0){
				// debugger;
			}
	...
	selfDeliver(amount){
		...
		if(traitsState.stored < 0) {traitsState.stored = 0;}
	},
	removeFromVisualStorage(id){
		traitsState.storedTraits = traitsState.storedTraits.filter(t => t.id != id);
		traitsState.updateVisualStorage();
	},
	...
	tick(isPaused){
		if(!traitsState.initComplete || isPaused || traitsState.tickInProgress){
			return;
		}
