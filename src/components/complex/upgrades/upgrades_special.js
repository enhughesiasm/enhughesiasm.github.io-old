...
const specialUpgrades = [
	{
		...
		'description': 'This is getting out of hand. Perhaps you need an actual factory..?',
		...
		apply: function(traitsState, activatePyro){
			...
			traitsState.getUpgradeByKey(upgradesKeys.TALES_OF_THE_PRELIFE).unlocked = true;
			traitsState.getUpgradeByKey(upgradesKeys.BETTER_MACHINERY).unlocked= true;
			if(activatePyro){
				activatePyro();
			}
	...
	},
	{
		...
		'description': 'Have a storyteller visit, and tell you stories from the rest of the prelife.',
		'explanation': '???',
		icon: 'comment',
		cost: 10000,
		unlocked: false,
		...
		apply: function(traitsState, activatePyro){
			traitsState.getUpgradeByKey(upgradesKeys.WONDER_ABOUT_LIFE).unlocked = true;
			if(activatePyro){
				activatePyro();
			}
			submitToAnalytics('upgrade', 'complex', 'TALES_OF_THE_PRELIFE - 1/5');
		}
	},
	{
		'key': upgradesKeys.WONDER_ABOUT_LIFE,
		'name': 'Tales of Earth',
		'description': 'This work is fun. You\'re helping people prepare to live their lives. But what else is out there...?',
		'explanation': '???',
		icon: 'question',
		cost: 1000000,
		unlocked: false,
		...
		apply: function(traitsState, activatePyro){
			traitsState.getUpgradeByKey(upgradesKeys.WORK_SATISFACTION).unlocked = true;
			if(activatePyro){
				activatePyro();
			}
	...
	},
	{
		...
		'description': 'Who cares what else there is? Surely throwing yourself into your work will be enough.',
		'explanation': '???',
		icon: 'question',
		cost: 10000000,
		unlocked: false,
		unlockedAtTraitsDelivered: 10000,
		apply: function(traitsState, activatePyro){
			traitsState.getUpgradeByKey(upgradesKeys.VISIT_THE_SHOP).unlocked = true;
			if(activatePyro){
				activatePyro();
			}
	...
	},
	{
		...
		'description': 'It\'d be nice to see what actually happens with all these traits, for once.',
		'explanation': '???',
		icon: 'question',
		cost: 100000000,
		unlocked: false,
		...
		apply: function(traitsState, activatePyro){
			traitsState.getUpgradeByKey(upgradesKeys.LEAVE_FOR_EARTH).unlocked = true;
			if(activatePyro){
				activatePyro();
			}
	...
	},
	{
		...
		'description': 'Leave your factory behind and head to the Shop Before Life—and then onto Earth. It\'s time for life to begin...',
		'explanation': '???',
		icon: 'globe-europe',
		cost: 25000000000,
		unlocked: false,
