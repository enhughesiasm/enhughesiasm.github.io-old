...
const ProdBuildings = (props) => {
	...
	let totalMachineWork = props.buildings.map(a => a.traitsOutputPerSecond).reduce((a,b) => a+b, 0);
	...
	return(<div id="prodBuildings" className="visualBuildings">
		<span
			style={{cursor: 'pointer'}}
			onClick={() => props.onReallocateProdWorker(employeeAllocations.MANUAL)}
			className={'building manualWorker tooltip is-tooltip-left is-tooltip-' + (currentProdWork > 0 ? 'success working' : 'warning notWorking')} data-tooltip={'Manual Production - ' + manualWorkers.allocation.manual + ' workers - ' + currentProdWork.toFixed(manualDp) + '/s'} >
		...
		{ props.buildings.map((b,x) => [...Array(b.builtAmount)].map( (building, y) =><span key={(x*y) + y} className={ 'prodBuilding ' + (y < b.workers ? 'buildingAllocated' : 'buildingUnallocated')}>
			{/* style={{animationDelay: Math.random() * 3 + 's'}} */}
			<span style={{cursor: 'pointer'}}
				onClick={() => props.onReallocateProdWorker(employeeAllocations.MACHINE)}
				className={'building tooltip is-tooltip-left is-tooltip-' + (y < b.workers ? 'success' : 'danger')} data-tooltip={b.name + ' - ' + (y < b.workers ? ((b.traitsOutputPerSecond * 1.0 / b.workers).toFixed(machineDp) + '/s') : ' 0/s - No worker!')} >
		...
		<OutputIndicator
			...
			amount={Number.parseFloat(((currentProdWork + totalMachineWork) || 0).toFixed(totalDp))}
