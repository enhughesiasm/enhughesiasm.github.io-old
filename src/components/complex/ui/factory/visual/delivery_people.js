...
const DeliveryPeople = (props) =>{
	...
	let anyLoaders = builtDeliveryBuildings?.filter(a => a.type == machineTypes.LOADER).length > 0;
	let loaders = {};
	if(anyLoaders){
		loaders = builtDeliveryBuildings.filter(a => a.type == machineTypes.LOADER)[0];
	}
	...
	let ratioType = ratio == ratios.BAD ? 'danger' : (ratio == ratios.OKAY ? 'warning' : 'success');

	let dp = (deliveryRate < 100 ? 2 : 0);

	return(<div id="deliveryPeople" className="visualBuildings">

		{anyLoaders && <span
			className={'building has-text-success loaders tooltip is-tooltip-left is-tooltip-success'} data-tooltip={loaders.builtAmount +  ' Loaders - Delivery speed x' + loaders.totalDeliveryMultiplier().toFixed(2) } >
		...
		<span
			style={{animationDuration: deliverers.deliveryAnimationDuration + 's'}}
			className={'building delivery tooltip is-tooltip-multiline is-tooltip-top is-tooltip-' + ratioType + ' is-delivery-' + ratioType}
			data-tooltip={deliverers.hiredAmount + ' ' + deliverers.name + ' making ' + deliveryRate.toFixed(dp) + ' deliveries per second'}>
