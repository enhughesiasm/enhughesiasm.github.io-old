...
const PowerBuildings = (props) => {
	...
	let totalGeneratedPower = props.buildings.map(b => (b.power * b.builtAmount) || 0).reduce((a,b) => a+b, 0);
	...
	return(<div id="powerBuildings" className="visualBuildings">
		{ actualBuildings.map( (building, y) =>	{

			let tooltipText = building.name + (building.amountUsed > 0 ? (' - Using ' + building.amountUsed + ' of ' + building.power) : ' - ' + building.power + ' Unused');

			return	(<span key={(building.x*y) + y}
				className={'powerBuilding building tooltip is-tooltip-left is-tooltip-' + (building.isUsed ? 'warning isUsed' : 'danger')}
		...
		<OutputIndicator
			...
			notificationType ={props.totalFactoryPower > 0 ? 'success' : 'warning'}
