...
//	useTraceUpdate(props);
	...
	const { storageNaturalSize, storageMachineSize, storageAmount, storageIsFull, machineStorageMultiplier } = props;
	...
	props.buildings.forEach(((b) => {
		for(let i = 0; i < b.builtAmount; i++){
			...
			let newB = {
				...
				storing: getStorage(currentStored, b.storage() * machineStorageMultiplier)
			};
	...
	return(<div id="storageBuildings" className="visualBuildings">
		{ buildings.map( (building) =>
			<span className={'storageBuilding building tooltip is-tooltip-left is-tooltip-' + (building.isFull ? 'warning isFull' : 'success') }
				key={building.id}
				data-tooltip={building.name + ' (' + (building.isFull ? building.size : building.storing) + '/' + building.size + ')'}
		...
		<OutputIndicator
			...
			amountText={storageAmount.toLocaleString().padStart(storageMagnitude, '0')}
			suffixText={' / ' + (storageNaturalSize + storageMachineSize).toLocaleString()}
		/>
