...
const MachinesBox = (props) =>
{
	...
	const { machinesAll, machinesUnlockedList, machinesGetCost, machinesAddToQueue, machinesSell, machinesCanBuild} = props;
	...
	for(let group in groupedMachines){
		machinesByRole.push(
			{ role: group,
				machines: groupedMachines[group]
			}
		);
	}

	return(<div className="tile is-child homeBox box has-text-centered is-paddingless cardList">
		<HelpText helpKey='machines' direction='left'/>
		<div className="subtitle is-size-6-mobile" style={{padding: '0 1.25rem'}}>MACHINES (INVENTED { machinesUnlockedList.length } / { (machinesAll.length-1) })</div>

		{ machinesByRole.map(role =>

			<div key={role.role}>
				<div className="is-divider" data-content={role.role.toUpperCase()}></div>
				{ role.machines.map(m => {
					...
					}else if (canBuild == machineBuildPermission.NOFAVOURS){
						buildTooltip = 'Not enough favours.';
					}

					let cost = machinesGetCost(m.type);

					return(<ComplexListEntry key={m.id}
						id={m.id}
						additionalClass="machine"
						icon={m.icon}
						name={ m.name + (m.builtAmount > 0 ? ( ' (' + m.builtAmount + ')') : '')}
						description={m.description}
						favoursAvailable={(canBuild!=machineBuildPermission.NOFAVOURS)}
						canPurchase={(canBuild==machineBuildPermission.SUCCESS)}
						cost={cost}
						onPurchaseClicked={() => machinesAddToQueue(m, cost)}
						sellText = { m.builtAmount > 0 ? 'Sell (' + m.builtAmount + ')' : ''}
						onSellClicked = {() => machinesSell(m)}
						justFailed={m.justFailed}
						tooltipData={(m.outcomePerSecond || 0).toFixed(2) + '/s'}
						purchaseText="Build"
						power={m.power}
