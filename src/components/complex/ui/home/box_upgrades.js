...
const BoxUpgrades = (props) => 	{
	...
	const { upgradesUnlocked, upgradesAll, upgradesCanPurchase, upgradesPurchase } = props;
	...
	return (<div className="tile is-child homeBox box upgrades cardList">
		<HelpText helpKey={'upgrades'} direction="left"/>
		<span className="subtitle is-size-6-mobile ">UPGRADES</span>
		<div id="upgradeBox">
			{ (upgradesUnlocked.length == 0 && remainingUpgrades > 0) && <div className="message">There&apos;s { remainingUpgrades } more...</div>}
			{ (upgradesUnlocked.length == 0 && remainingUpgrades == 0) && <div className="message"><FontAwesome icon={'handshake' }/> COMPLETE <FontAwesome icon={'glass-cheers' }/></div>}
			{ upgradesUnlocked.map(upgrade => {
				let canPurchase = upgradesCanPurchase(upgrade.id);
				return(	<ComplexListEntry key={upgrade.id}
					id={upgrade.id}
					...
					purchaseText="Buy"
					onPurchaseClicked={() => upgradesPurchase(upgrade.id)}/>);
