...
const FriendlyNumber = (props) => {
	...
	return(<span className={'is-size-7-mobile ' + (props.amount > 999999 ? ('tooltip is-tooltip-' + direction) : '')} data-tooltip={props.amount + (props.name ? (' ' + props.name) : '')} >
		{ (props.prefix || '') + display + suffix } { props.icon && <FontAwesome icon={props.icon}/> }
	</span>);};
