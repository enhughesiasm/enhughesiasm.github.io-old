...
import subscribePopupStorage from '../shared/subscribe_popup/subscribe_popup_storage';
...
class MailchimpSignup extends React.Component {
	...
	handleSubmit = async (e) => {
		...
		if (response.ok) {
			...
			// TK share with subscribe_popup.js
	...
	render() {
		...
		return (
			<>
				{!this.state.submitSent && (
					<form id='mailchimpForm' onSubmit={this.handleSubmit}>
						<input
							type='hidden'
							name='u'
							...
							<div className='field'>
								...
								<div
									...
									<input
										className={
											'input is-rounded ' +
											(this.state.fNameValue.length >= 2
												? 'is-success'
												: 'is-warning')
										}
										type='text'
								...
								<div className='content'>
									<div className='content__gdprLegal'>
										<p className='smallPrint'>
											Mailchimp is my email list
											management platform. By clicking
											&apos;subscribe&apos;, you
											acknowledge that your information
											will be processed according to the
											terms on this page, and transferred
				...
				{this.state.submitSent &&
					...
					!this.state.alreadySubscribed && (
						<div className='notification is-light has-text-centered'>
							<LoadingBounce size='50' />
						</div>
					)}
				{this.state.submitSent && this.state.submitSuccess && (
					<Redirect to={'/confirm-signup'} />
				)}
				{this.state.submitSent && this.state.submitFailed && (
					<div className='notification is-danger has-text-centered'>
						<h2>Sorry!</h2>
						<p>
							Something went wrong.{' '}
							<a href='http://eepurl.com/gB3e8P'>
								Click here to try again.
							</a>
						</p>
					</div>
				)}
				{this.state.submitSent && this.state.alreadySubscribed && (
					<div className='notification is-warning has-text-centered'>
						<h2>Oops!</h2>
						<p>
							That email was already subscribed. Your email
							consent has been recorded. To receive another
							subscription confirmation (which contains a link to
