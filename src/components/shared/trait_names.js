...
const positiveFraction = 0.6;
const preferencesFraction = 0.25;
...
export function getTraitName() {
	...
	if (x < positiveFraction) {
		trait =
			getAdjective(adjectiveFraction) +
			positives[Math.floor(Math.random() * positives.length)];
	} else if (x < positiveFraction + preferencesFraction) {
		trait =
			getAdjective(preferenceAdjectiveFraction) +
			preferenceVerbs[
				Math.floor(Math.random() * preferenceVerbs.length)
			] +
