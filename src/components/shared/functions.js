...
export function orderByMultipleProperties(prop) {
	...
	return function (a, b) {
		...
		if(Number.isInteger(a[prop]) && Number.isInteger(b[prop])){
			...
			equality = (a[prop] || '').toString().localeCompare((b[prop] || '').toString());
		}
...
export function groupBy(array, fnSelectKey) {
	return array.reduce((r, v, i, a, k = fnSelectKey(v)) => ((r[k] || (r[k] = [])).push(v), r), {});
}
...
export const hasBodyClass = className => document.body.classList.contains(className);