...
class QuoteCycler extends React.Component {
	...
	constructor(props){
		...
		this.state =
		{
			...
			opacity: 1,
			millisecondsPerTransition:  ((this.props.secondsPerTransition) || 1.5) * 1000,
			millisecondsPerQuote: (this.props.secondsPerQuote || (2 + (Math.random() * 3)))  * 1000,
			isTransitioning: false,
			currentQuoteProgressFraction: 0,
			quotes: this.props.quotes.map((a) => ({sort: Math.random(), value: a})).sort((a, b) => a.sort - b.sort).map((a) => a.value)
		};
	...
	tick(){
		...
		if(this.state.isTransitioning){
			...
			if(currentQuoteDuration > this.state.millisecondsPerQuote){
				let nextIndex = (this.state.currentQuoteIndex < (this.state.quotes.length -1))
	...
	render(){
		...
		return 	<div>
			{showLoader && <a className="smallPrint is-pulled-left blockquoteToggle"
				onClick={this.toggleShowAllQuotes}>
				{ this.state.showAllQuotes ? 'hide ' : 'show all ' + this.state.quotes.length} quotes
			</a>
			}
			{ !this.state.showAllQuotes &&
			<Quote showLoader={showLoader}
				opacity={this.state.opacity}
				quote={this.state.quotes[this.state.currentQuoteIndex].quote}
				cite={this.state.quotes[this.state.currentQuoteIndex].cite}
			/>
			}
			{ this.state.showAllQuotes &&
			<div>
				{ this.state.quotes.map((quote, index) =>
					<Quote key={getHashCode(this.state.quotes[index].quote)}
						showLoader={index==0}
